...
peelTheme <- sp.theme( regions= list( col= legend[ peelClasses +1]))
...
thumbDfPlot <- spplot(thumbDf, c("pri", "sec"))
...
thumbFractions <- 
  sapply( names( reclassNames), 
         function( cover) {
           reclassFractions( thumb, reclassNames[[cover]], 
...
thumbStack <- 
  do.call( stack, 
          c( myUnlist( thumb), 
             ...
             thumbCheck))
layerNames(thumbStack) <- c("pri", "sec", "pct", "Ap", names(reclassNames), "check")
