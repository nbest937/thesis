...
\begin{abstract} This paper presents an effort to produce a new land
  ...
  set has lower resolution than the MLCT (5 arcminutes vs. 500m) but
  represents consituent cover classes as sub-pixel fractions rather
  than forcing individual pixels into mutually exclusive thematic
  categories.  After aggregating to the 5-arcmin grid we present a
  method for decomposing the natural vegetaion/cropland mosaic class
  found in MLCT into constituent classes as a function of the local
  landscape and quantify its contribution to aggregate acreages by
  class, particularly cropland.  We compensate for the absence of
  certain fine-grained details from MLCT, such as rural transportation
  networks, small settlements, linear water features, and wetlands,
  mainly by virtue of sensor resolution, by incorporating
  ...
  final classification scheme, now conceptually analogous to a stack
  of spectral bands that are constrained to sum to unity, is a mixture
  of a simplified version of the IGBP schema used in MLCT and a
  disaggregation of the monolithic cropland class that differentiates
  among the world's major commodity crops. \todo{It remains to be seen
    how well the disaggregation of commodities will work. We may have
    to leave it out.}  At each step of refinement we show that overall
  spatial distribution of cropland across the study area improves
  relative to the Aglands2000 data set.  We close with a discussion of
  how this method might be applied globally and to successive years in
  the MLCT time series.
\end{abstract}
