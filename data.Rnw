...
igbpLegend <- getColorTable(mlct)[ getColorTable(mlct) != "#000000"]
...
## calculate the fractions for A_{min}=1.0 from scratch
...
##       primaryFraction(
##         mlctReclass( 
##           mlctList( "thumb_2001_lct1.tif", 
##                     "thumb_2001_lct1_sec.tif", 
##                     "thumb_2001_lct1_pct.tif"),
##           mlctReclassMatrix),
##         1.0)))

## alternatively, just reuse the existing data 
## loading and reclassifying the data are redundant

thumbPri <- 
  aggregateFractions(
    coverFractions(
      primaryFraction( thumb, 1.0)))
...
p <- ggplot( data= as.data.frame( as( sampleRegular( thumb$pri, ncell(thumb$pri)*0.05, asRaster=TRUE), "SpatialGridDataFrame")))

ggplotRaster <- function( r, frac) {
  df <- data.frame( as( sampleRegular( r, ncell( r)*frac, 
                                      asRaster=TRUE), 
...
peelMap <- function( r, frac) {
  p <- ggplotRaster( r, frac)
