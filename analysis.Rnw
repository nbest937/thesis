...
\SweaveOpts{ include=false, echo=false, engine=R}
...
Aglands2000 indicates roughly \Sexpr{ printAreas( areasDf[ "ag",
  "crop"]) } of cropland.
...
# cite Sarkar, sec. 6.2.3
trellis.par.set( divTheme)
ord <- order.dendrogram(as.dendrogram(hclust(dist(diffCor))))
levelplot(diffCor[ord, ord], at=do.breaks(c(-1.01,1.01),20), xlab="", ylab="")
@ 
\end{center} 
