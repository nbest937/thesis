...
thumb  <- primaryFraction( thumb, Amin=0.5, overwrite= overwriteRasters)
thumb1 <- primaryFraction( thumb, Amin=1.0, overwrite= overwriteRasters)
thumb  <- coverFractions( thumb, overwrite= overwriteRasters)
thumb1 <- coverFractions( thumb1, overwrite= overwriteRasters)
thumb  <- aggregateFractions( thumb, overwrite= overwriteRasters)
thumb1 <- aggregateFractions( thumb1, overwrite= overwriteRasters)
...
if( overwriteFigures) {
  quartz( file="fig_thumb_fracs.png")
...
if( overwriteFigures) {
  quartz( file="fig_thumb1_fracs.png")
...
if( overwriteFigures) {
  quartz( file="fig_thumb1_agg.png")
...
if( overwriteFigures || TRUE) {
  quartz( file="fig_thumb_agg_diff.png")
...
thumb  <- decomposeMosaic( thumb, overwrite= overwriteRasters)
thumb1 <- decomposeMosaic( thumb1, overwrite= overwriteRasters)
...
if( overwriteFigures) {
  quartz( file="fig_thumb_nomos.png")
...
if( overwriteFigures) {
  quartz( file="fig_thumb1_nomos.png")
...
if( overwriteFigures  || TRUE) {
  quartz( file="fig_thumb_nomos_diff.png")
...
mlct  <- primaryFraction( mlct, Amin= 0.5, overwrite= overwriteRasters)
mlct  <- coverFractions( mlct, overwrite= overwriteRasters)
mlct  <- aggregateFractions( mlct, overwrite= overwriteRasters)
mlct  <- decomposeMosaic( mlct, overwrite= overwriteRasters)

mlct1  <- primaryFraction( mlct, Amin=1.0, overwrite= overwriteRasters)
mlct1  <- coverFractions( mlct1, overwrite= overwriteRasters)
mlct1  <- aggregateFractions( mlct1, overwrite= overwriteRasters)
mlct1  <- decomposeMosaic( mlct1, overwrite= overwriteRasters)
...
thumbNlcd <- mlctReclass( thumbNlcd, nlcdReclassMatrix, overwrite=TRUE)

thumbNlcdPlot <- peelMap(thumbNlcd$pri, 0.05)


thumbNlcd$Amin <- 1
thumbNlcd <- coverFractions( thumbNlcd, overwrite=TRUE, progress="text")
thumbNlcd <- aggregateFractions( thumbNlcd, overwrite=TRUE, progress="text")
