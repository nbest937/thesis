...
mlct  <- primaryFraction( mlct, Amin= 0.5, overwrite= overwriteRasters || TRUE, progress="text")
mlct  <- coverFractions( mlct, overwrite= overwriteRasters || TRUE, progress="text")
...
thumbAdj  <- decomposeMosaic( thumbAdj, overwrite= overwriteRasters || TRUE, progress= "text")
